    ...
    var RESOURCE_CONFIGS = {
        ...
        disciplinas: {
            ...
            fields: [
                ...
                { name: 'area_conhecimento', label: 'Área do conhecimento', type: 'text', col: 'col-md-8' },
                { name: 'ativo', label: 'Disciplina ativa', type: 'checkbox', col: 'col-md-4' },
            ],
            columns: [
                ...
                { label: 'Área', render: function (item) { return escapeHtml(item.area_conhecimento || 'Não informada'); } },
                { label: 'Carga', render: function (item) { return escapeHtml(item.carga_horaria_padrao ? String(item.carga_horaria_padrao) + 'h' : 'Não definida'); } },
        ...
        series: {
            ...
            columns: [
                ...
                { label: 'Etapa', render: function (item) { return escapeHtml(item.etapa || 'Não informada'); } },
                { label: 'Faixa etária', render: function (item) { return escapeHtml((item.idade_minima != null ? item.idade_minima + ' anos' : 'Sem mínimo') + ' • ' + (item.idade_maxima != null ? item.idade_maxima + ' anos' : 'Sem máximo')); } },
                { label: 'Status', render: function (item) { return badge(item.ativo !== false ? 'Ativa' : 'Inativa', item.ativo !== false ? 'success' : 'secondary'); } },
            ],
        },
        turnos: {
            ...
            fields: [
                ...
                { name: 'ativo', label: 'Turno ativo', type: 'checkbox', col: 'col-md-6' },
            ],
            columns: [
                ...
                { label: 'Faixa horária', render: function (item) { return escapeHtml((item.hora_inicio || 'N/I') + ' → ' + (item.hora_fim || 'N/I')); } },
                { label: 'Carga', render: function (item) { return escapeHtml(item.carga_horaria_minutos ? String(item.carga_horaria_minutos) + ' min' : 'Não definida'); } },
        ...
        calendarios: {
            ...
            columns: [
                ...
                { label: 'Vigência', render: function (item) { return escapeHtml((item.data_inicio || 'N/I') + ' → ' + (item.data_fim || 'N/I')); } },
        ...
        apiFetch('/api/institucional/parametros').then(function (data) {
            ...
            if (statsContainer) {
                statsContainer.innerHTML = [
                    ...
                    { label: 'Nota mínima', value: data.nota_minima || 0 },
